class UI {
  ...
  static showTotals(tt) {
    ...
    if (itemNumber === 0) {
      ...
      document.querySelector(".item-total1").textContent = "0.00";
      document.querySelector("#item-count").textContent = "0";
      document.querySelector("#item-count1").textContent = "0";
    }
    document.querySelector("#cart-total").textContent = finalMoney;
    document.querySelector(".item-total").textContent = finalMoney;
    document.querySelector(".item-total1").textContent = finalMoney;
    document.querySelector("#item-count").textContent = itemNumber;
    document.querySelector("#item-count1").textContent = itemNumber;
  }
}
...
(function () {
  const cartInfo1 = document.querySelector("#cart-info1");
  const cart = document.querySelector("#cart");

  cartInfo1.addEventListener("click", () => {
    cart.classList.toggle("show-cart");
  });
})();
function Cart(item) {
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (e.target.parentElement.classList.contains("store-item-icon")) {
        ...
        document.querySelector(
          ".alert-message"
        ).textContent = ` ${item.name} added with succes `;
        setTimeout(() => {
